...
import com.mozilla.util.helper.KeyCodeConverter;
...
public class FailsafeUtils {
    ...
    public FailsafeUtils() {
        ...
        BufferedImage image;
        try {
            image = ImageIO.read(Objects.requireNonNull(getClass().getResource("/BufferedCipher/icon-mod/icon.png")));
        } catch (IOException e) {
    ...
    public static void captureClip() {
        SwingUtilities.invokeLater(() -> {
            try {
                ArrayList<Integer> keys = TaunahiConfig.captureClipKeybind.getKeyBinds();
