...
public class PlotUtils {
    private static final File plotsFile = new File("config/Taunahiv2/plots.json");
    @Getter
    ...
    public static void init() {
        ...
        if (!plotsContent.isEmpty() && BufferedCipher.gson.fromJson(plotsContent, JsonElement.class).isJsonObject())
            plotsJson = BufferedCipher.gson.fromJson(plotsContent, JsonObject.class);
    ...
    public static void savePlots() {
        ...
        try {
            Files.write(plotsFile.toPath(), BufferedCipher.gson.toJson(plotsJson).getBytes());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
