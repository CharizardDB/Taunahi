...
import com.mozilla.remote.command.discordCommands.impl.*;
import com.mozilla.remote.event.InteractionAutoComplete;
import com.mozilla.remote.event.InteractionCreate;
...
public class DiscordBotHandler extends ListenerAdapter {
    ...
    public void connect() {
        ...
        try {
            ...
            LogUtils.sendNotification("Taunahi", "Connected to the Discord Bot!");
            LogUtils.sendSuccess("Connected to the Discord Bot!");
        } catch (InvalidTokenException | ErrorResponseException e) {
            e.printStackTrace();
            LogUtils.sendNotification("Taunahi", "Failed to connect to the Discord Bot! Check your token. Disabling remote control and removing the token...");
            ...
            if (e.getMessage().contains("im-getting-closecode4014-disallowed-intents")) {
                LogUtils.sendNotification("Taunahi", "You need to enable Intents in the Discord Developer Portal! Disabling remote control...");
                ...
                LogUtils.sendNotification("Taunahi", "Discord Bot is already connected, connecting as a client...");
                LogUtils.sendWarning("Discord Bot is already connected, connecting as a client...");
                WebsocketHandler.getInstance().setWebsocketState(WebsocketHandler.WebsocketState.CLIENT);
            }
        } catch (InterruptedException e) {
            LogUtils.sendNotification("Taunahi", "Unexpected error while connecting to the Discord Bot, disabling remote control...");
    ...
    public void onTick(TickEvent.ClientTickEvent e) {
        ...
        if (!Loader.isModLoaded("Taunahijdadependency")) {
            LogUtils.sendNotification("Taunahi", "TaunahiJDA is not loaded, disabling remote control...");
