...
import com.mozilla.feature.impl.AntiStuck;
...
public class TeleportFailsafe extends Failsafe {
    ...
    public void onReceivedPacketDetection(ReceivePacketEvent event) {
        ...
        if (distance >= TaunahiConfig.teleportDistanceThreshold || (MacroHandler.getInstance().getCurrentMacro().isPresent() && Math.abs(packet.getY()) - Math.abs(MacroHandler.getInstance().getCurrentMacro().get().getLayerY()) > 0.8)) {
            ...
            if (lastReceivedPacketDistance > 7.5D && Math.abs(lastReceivedPacketDistance - estimatedMovement) < TaunahiConfig.teleportLagTolerance)
                return;
            if (originalPosition == null) {
                originalPosition = mc.thePlayer.getPositionVector();
            }
            triggerCheck.schedule(TaunahiConfig.detectionTimeWindow);
        }
    }
    ...
    private void evaluateDisplacement() {
        ...
        if (totalDisplacement > TaunahiConfig.teleportDistanceThreshold) {
