package com.mozilla.pathfinder;
...
import com.mozilla.mixin.client.EntityPlayerAccessor;
import com.mozilla.mixin.pathfinder.PathfinderAccessor;
import com.mozilla.util.*;
import com.mozilla.util.helper.*;
...
public class FlyPathFinderExecutor {
    ...
    public void onTickNeededYaw(TickEvent.ClientTickEvent event) {
        ...
        if (checkForStuck(current)) {
            ...
            if (TaunahiConfig.flyPathfinderOringoCompatible) {
            ...
            if (!this.dontRotate && target.getTarget().isPresent() && !path.isEmpty()) {
                ...
                if (mc.thePlayer.getPositionVector().distanceTo(lastElement) > 2 && target.getTarget().isPresent() && RotationHandler.getInstance().shouldRotate(rot, 3)) {
                    ...
                    RotationHandler.getInstance().easeTo(new RotationConfiguration(
                            rot,
                            (long) (TaunahiConfig.getRandomFlyPathExecutionerRotationTime() * (Math.max(1, distanceTo / 90))),
                            null
                    ));
                }
            }

            if (TaunahiConfig.useAoteVInPestsDestroyer && tped && useAOTV && aotvDelay.passed() && mc.thePlayer.getDistance(next.xCoord, mc.thePlayer.getPositionVector().yCoord, next.zCoord) > 12 && !RotationHandler.getInstance().isRotating() && isFrontClean()) {
    ...
    private Vec3 predictStoppingPosition() {
        ...
        playerSimulation.rotationYaw = neededYaw != Integer.MIN_VALUE && !TaunahiConfig.flyPathfinderOringoCompatible ? neededYaw : mc.thePlayer.rotationYaw;
        for (int i = 0; i < 30; i++) {
