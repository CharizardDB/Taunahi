...
public class AutoSell implements IFeature {
    ...
    public void onTickShouldEnable(TickEvent.ClientTickEvent event) {
        ...
        if (TaunahiConfig.pauseAutoSellDuringJacobsContest && GameStateHandler.getInstance().inJacobContest())
            return;
    ...
    private boolean shouldSellCustomItem(String name) {
        if (TaunahiConfig.autoSellRunes && name.contains(" Rune") && !name.contains("Music")) return true;
        if (TaunahiConfig.autoSellDeadBush && name.contains("Dead Bush")) return true;
        if (TaunahiConfig.autoSellIronHoe && name.contains("Iron Hoe")) return true;
        if (TaunahiConfig.autoSellPestVinyls && name.contains("Vinyl")) return true;
        if (!TaunahiConfig.autoSellCustomItems.isEmpty()) {
            List<String> customItems = Arrays.asList(TaunahiConfig.autoSellCustomItems.split("\\|"));
