...
import com.mozilla.util.FailsafeUtils;
...
public class BanInfoWS implements IFeature {
    ...
    public static final File statsDirectory = new File(Minecraft.getMinecraft().mcDataDir, "Taunahi");
    public static final String fileNamePrefix = "th_stats_";
    ...
    private void processBanScreen(String wholeReason) {
        ...
        try {
            ...
            if (TaunahiConfig.captureClipAfterFailsafe && !TaunahiConfig.captureClipKeybind.getKeyBinds().isEmpty()) {
                Multithreading.schedule(() -> {
                    FailsafeUtils.captureClip();
                    LogUtils.sendDebug("[Failsafe] Clip captured!");
                }, TaunahiConfig.captureClipDelay, TimeUnit.SECONDS);
            }
    ...
    public void playerBanned(int days, String reason, String banId, String fullReason) {
        ...
        jsonObject.addProperty("mod", "Taunahi");
        ...
        String config = BufferedCipher.config.getJson();
        JsonObject configJson = BufferedCipher.gson.fromJson(config, JsonObject.class);
        ...
        String configJsonString = BufferedCipher.gson.toJson(configJson);
    ...
    private void collectMods(JsonObject obj) {
        ...
        try {
            ...
            File TaunahiFile = new File(oneConfigFolder, "BufferedCipher/config.json");
            if (TaunahiFile.exists()) {
                configFiles.addProperty("BufferedCipher", compress(TaunahiFile));
            }
        ...
        JsonObject Taunahi = new JsonObject();
        Taunahi.addProperty("macroEnabled", MacroHandler.getInstance().isMacroToggled());
        Taunahi.addProperty("pestsDestroyerEnabled", TaunahiConfig.enablePestsDestroyer);
        Taunahi.addProperty("crop", MacroHandler.getInstance().getCrop().name());
        Taunahi.addProperty("macroType", TaunahiConfig.getMacro().name());
        Taunahi.addProperty("fastBreak", TaunahiConfig.fastBreak);
        Taunahi.addProperty("lastFailsafes", FailsafeManager.getInstance().getBanInfoWSLastFailsafe());
        Taunahi.addProperty("longestSessionLast7D", getLongestSessionLast7D());
        Taunahi.addProperty("autoCookie", TaunahiConfig.autoCookie);
        Taunahi.addProperty("autoGodPot", TaunahiConfig.autoGodPot);
        extraData.add("BufferedCipher", Taunahi);
    ...
    public void saveStats() {
        ...
        jsonObject.addProperty("modVersion", BufferedCipher.VERSION);
        jsonObject.addProperty("timeMacroing", MacroHandler.getInstance().getAnalyticsTimer().getElapsedTime());
        jsonObject.addProperty("fastBreak", TaunahiConfig.fastBreak);
    ...
    public void sendFailsafeInfo(FailsafeManager.EmergencyType type) {
        ...
        jsonObject.addProperty("mod", "BufferedCipher");
        jsonObject.addProperty("username", Minecraft.getMinecraft().getSession().getUsername());
        jsonObject.addProperty("id", Minecraft.getMinecraft().getSession().getPlayerID());
        jsonObject.addProperty("modVersion", BufferedCipher.VERSION);
    ...
    private JsonObject getHeaders() {
        ...
        handshake.addProperty("modVersion", BufferedCipher.VERSION);
        handshake.addProperty("mod", "BufferedCipher");
        try {
            String serverId = mojangAuthentication();
    ...
    private WebSocketClient createNewWebSocketClient() throws URISyntaxException {
        return new WebSocketClient(new URI("")) {
            @Override
            ...
            public void onMessage(String message) {
                JsonObject jsonObject = BufferedCipher.gson.fromJson(message, JsonObject.class);
                ...
                switch (msg) {
                    case "banwaveInfo": {
                        ...
                        if (!receivedBanwaveInfo) {
                            if (client.isOpen() && client.getReadyState() != ReadyState.NOT_YET_CONNECTED) {
                                ...
                                LogUtils.sendNotification("Taunahi", "Connected to analytics websocket server");
                            }
                        }
                    ...
                    case "playerGotBannedWithStats": {
                        ...
                        // LogUtils.sendNotification("Taunahi", "User " + username + " got banned for " + days + " days");
                        break;
                    }
                }
            }
