...
public class PiPMode implements IFeature {
    ...
    public void setAlwaysOnTop(boolean enabled) {
        try {
            ...
            user32.EnumWindows((hWnd, data) -> {
                ...
                if (title.contains("Taunahi") && title.contains(mc.getSession().getUsername())) {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        if (!TaunahiConfig.pipMode || event.phase == TickEvent.Phase.END || mc.thePlayer == null || mc.theWorld == null || !isRunning())
            return;
