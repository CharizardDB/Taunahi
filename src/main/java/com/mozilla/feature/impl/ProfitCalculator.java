...
import com.mozilla.event.UpdateScoreboardLineEvent;
import com.mozilla.failsafe.impl.LowerAvgBpsFailsafe;
import com.mozilla.feature.IFeature;
import com.mozilla.handler.GameStateHandler;
import com.mozilla.handler.MacroHandler;
import com.mozilla.util.APIUtils;
...
public class ProfitCalculator implements IFeature {
    ...
    @Override
    public void start() {
        if (TaunahiConfig.resetStatsBetweenDisabling) {
            resetProfits();
        }
    ...
    public void onTickUpdateProfit(TickEvent.PlayerTickEvent event) {
        ...
        if (currentItem != null && currentItem.getItem() != null && TaunahiConfig.profitCalculatorCultivatingEnchant) {
        ...
        if (TaunahiConfig.countRNGToProfitCalc) {
    ...
    public void onReceivedPacket(ReceivePacketEvent event) {
        ...
        if (event.packet instanceof S2FPacketSetSlot) {
            ...
            if (TaunahiConfig.profitCalculatorCultivatingEnchant && newItem != null && heldItem != null && StringUtils.stripControlCodes(newItem.getDisplayName()).equals(MacroHandler.getInstance().getCrop().getLocalizedName()) && GameStateHandler.getInstance().getCurrentCultivating().getOrDefault(heldItem.getDisplayName(), 0L) > 0) {
                return;
            }
    ...
    public static class BazaarItem {
        ...
        public BazaarItem setImage() {
            this.imageURL = "/BufferedCipher/textures/gui/" + getImageName(localizedName);
            return this;
        }
    }
