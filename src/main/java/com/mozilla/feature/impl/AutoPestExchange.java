...
public class AutoPestExchange implements IFeature {
    ...
    private BlockPos deskPos() {
        return new BlockPos(TaunahiConfig.pestExchangeDeskX, TaunahiConfig.pestExchangeDeskY, TaunahiConfig.pestExchangeDeskZ);
    }

    private boolean isDeskPosSet() {
        return TaunahiConfig.pestExchangeDeskX != 0 && TaunahiConfig.pestExchangeDeskY != 0 && TaunahiConfig.pestExchangeDeskZ != 0;
    }
    ...
    public boolean isToggled() {
        return TaunahiConfig.autoPestExchange;
    }
    ...
    public boolean canEnableMacro(boolean manual) {
        ...
        if (!manual && TaunahiConfig.pauseAutoPestExchangeDuringJacobsContest && GameStateHandler.getInstance().inJacobContest()) {
        ...
        if (!manual && !GameStateHandler.getInstance().inJacobContest() && !TaunahiConfig.autoPestExchangeIgnoreJacobsContest) {
            ...
            for (String line : TablistUtils.getTabList()) {
                ...
                if (matcher.find() && matcher.groupCount() >= 1) {
                    ...
                    if (minutes > TaunahiConfig.autoPestExchangeTriggerBeforeContestStarts) {
        ...
        if (!manual && !TaunahiConfig.autoPestExchangeIgnoreJacobsContest && TaunahiConfig.autoPestExchangeOnlyStartRelevant) {
            if (!GameStateHandler.getInstance().getJacobsContestNextCrop().contains(MacroHandler.getInstance().getCrop())) {
                LogUtils.sendDebug("[Auto Pest Exchange] The current crop (" + MacroHandler.getInstance().getCrop().getLocalizedName() +
                        ...
                        Strings.join(GameStateHandler.getInstance().getJacobsContestNextCrop().stream().map(TaunahiConfig.CropEnum::getLocalizedName).collect(Collectors.toList()), ", ") +
                        "), skipping...");
                return false;
            }
        }
        if (!manual && GameStateHandler.getInstance().getPestsFromVacuum() < TaunahiConfig.autoPestExchangeMinPests) {
    ...
    public void onTickExecutionNew(TickEvent.ClientTickEvent event) {
        ...
        if (stuckClock.isScheduled() && stuckClock.passed()) {
            ...
            TaunahiConfig.autoPestExchange = false;
        ...
        switch (newState) {
            ...
            case GO_TO_DESK:
                ...
                if (isDeskPosSet()) {
                    if (!TaunahiConfig.autoPestExchangeTravelMethod) {
            ...
            case FIND_PHILLIP:
                ...
                phillip = getPhillip();
                if (phillip == null) {
                    if (!FlyPathFinderExecutor.getInstance().isRunning() && !TaunahiConfig.autoPestExchangeTravelMethod) {
                        FlyPathFinderExecutor.getInstance().setSprinting(false);
                        FlyPathFinderExecutor.getInstance().setDontRotate(true);
                        FlyPathFinderExecutor.getInstance().findPath(new Vec3(initialDeskPos).addVector(0.5f, 0.5f, 0.5f), true, true);
                    }
                    if (!BaritoneHandler.isPathing() && TaunahiConfig.autoPestExchangeTravelMethod) {
                ...
                TaunahiConfig.pestExchangeDeskX = closestPos.getX();
                TaunahiConfig.pestExchangeDeskY = closestPos.getY();
                TaunahiConfig.pestExchangeDeskZ = closestPos.getZ();
            ...
            case EMPTY_VACUUM:
                ...
                if (invName.contains("Pesthunter")) {
                    ...
                    if (lore.stream().anyMatch(l -> l.contains("Click to empty"))) {
                        if (TaunahiConfig.logAutoPestExchangeEvents)
    ...
    public void onRenderWorldLast(RenderWorldLastEvent event) {
        if (!isRunning()) return;
        if (mc.thePlayer == null || mc.theWorld == null) return;
        if (TaunahiConfig.streamerMode) return;
        if (!TaunahiConfig.highlightPestExchangeDeskLocation) return;
        if (TaunahiConfig.pestExchangeDeskX == 0
                && TaunahiConfig.pestExchangeDeskY == 0
                && TaunahiConfig.pestExchangeDeskZ == 0) return;
