...
public class VisitorsMacro implements IFeature {
    ...
    public void resetStatesAfterMacroDisabled() {
        ignoredNPCs.clear();
        if (!TaunahiConfig.visitorsMacroAfkInfiniteMode) return;
        TaunahiConfig.visitorsMacroAfkInfiniteMode = false;
    ...
    public boolean canEnableMacro(boolean manual, boolean withError) {
        ...
        if (!manual && TaunahiConfig.pauseVisitorsMacroDuringJacobsContest && GameStateHandler.getInstance().inJacobContest()) {
        ...
        if (GameStateHandler.getInstance().getCurrentPurse() < TaunahiConfig.visitorsMacroMinMoney * 1_000) {
            ...
            if (TaunahiConfig.visitorsMacroAfkInfiniteMode) {
        ...
        if (visitorsWithoutServed.size() < TaunahiConfig.visitorsMacroMinVisitors) {
    ...
    private void onVisitorsState() {
        switch (visitorsState) {
            case NONE:
                if (visitors.isEmpty() && TaunahiConfig.visitorsMacroAfkInfiniteMode) {
            ...
            case GET_CLOSE_TO_VISITOR:
                ...
                if (TaunahiConfig.visitorsExchangeTravelMethod && BaritoneHandler.isWalkingToGoalBlock()) {
                    return;
                }
                if (!TaunahiConfig.visitorsExchangeTravelMethod && FlyPathFinderExecutor.getInstance().isRunning()) {
                ...
                if (mc.thePlayer.getDistanceToEntity(currentCharacter.get()) > 3) {
                    ...
                    if (TaunahiConfig.visitorsExchangeTravelMethod)
            ...
            case CHECK_VISITOR:
                ...
                if (profitNpc && TaunahiConfig.sendVisitorsMacroLogs) {
                ...
                if (TaunahiConfig.filterVisitorsByName) {
                    if (TaunahiConfig.nameFilter.isEmpty()) {
                        ...
                        List<String> visitorsList = Arrays.asList(TaunahiConfig.nameFilter.split("\\|"));
                ...
                if (TaunahiConfig.filterVisitorsByRarity && !rejectVisitor && !ignoredNPCs.contains(currentVisitor.get()) && !acceptWhitelistedVisitor) {
                    switch (npcRarity) {
                        case UNKNOWN:
                        ...
                        case UNCOMMON:
                            ...
                            } else if (TaunahiConfig.visitorsActionUncommon == 3) {
                        ...
                        case RARE:
                            if (TaunahiConfig.visitorsActionRare == 0) {
                                ...
                                checkIfCurrentVisitorIsProfitable();
                            } else if (TaunahiConfig.visitorsActionRare == 3) {
                                LogUtils.sendDebug("[Visitors Macro] The visitor is uncommon rarity. Ignoring...");
                                ignoredNPCs.add(currentVisitor.get());
                            } else {
                        ...
                        case LEGENDARY:
                            if (TaunahiConfig.visitorsActionLegendary == 0) {
                                ...
                                checkIfCurrentVisitorIsProfitable();
                            } else if (TaunahiConfig.visitorsActionLegendary == 3) {
                                LogUtils.sendDebug("[Visitors Macro] The visitor is uncommon rarity. Ignoring...");
                                ignoredNPCs.add(currentVisitor.get());
                            } else {
                        ...
                        case MYTHIC:
                            if (TaunahiConfig.visitorsActionMythic == 0) {
                                ...
                                checkIfCurrentVisitorIsProfitable();
                            } else if (TaunahiConfig.visitorsActionMythic == 3) {
                                LogUtils.sendDebug("[Visitors Macro] The visitor is uncommon rarity. Ignoring...");
                                ignoredNPCs.add(currentVisitor.get());
                            } else {
                        ...
                        case SPECIAL:
                            if (TaunahiConfig.visitorsActionSpecial == 0) {
                                ...
                                checkIfCurrentVisitorIsProfitable();
                            } else if (TaunahiConfig.visitorsActionSpecial == 3) {
                                LogUtils.sendDebug("[Visitors Macro] The visitor is uncommon rarity. Ignoring...");
                                ignoredNPCs.add(currentVisitor.get());
                            } else {
    ...
    private Vec3 noClosestVecFallback(Vec3 closestVec) {
        ...
        if (closestVec == null) {
            ...
            if (closestVec == null) {
                ...
                TaunahiConfig.visitorsMacro = false;
                return null;
            }
        }
        ...
        switch (buyState) {
            case NONE:
                ...
                if (itemsToBuy.get(0).getLeft().toLowerCase().contains("cake") && !InventoryUtils.canFitCakesInInventory(itemsToBuy.get(0).getRight())) {
                    ...
                    itemsToBuy.remove(0);
                    if (TaunahiConfig.fullInventoryAction)
                        ignoredNPCs.add(currentVisitor.get());
                    else
                        rejectVisitor = true;
                    return;
                ...
                break;
            case BUY_ITEMS:
                AutoBazaar.getInstance().buy(itemsToBuy.get(0).getLeft(), itemsToBuy.get(0).getRight(), TaunahiConfig.visitorsMacroMaxSpendLimit * 1_000_000);
