...
import com.mozilla.util.RenderUtils;
...
public class MacroHandler {
    ...
    public void toggleMacro() {
        if (isMacroToggled()) {
            ...
            if (VisitorsMacro.getInstance().isRunning() || TaunahiConfig.visitorsMacroAfkInfiniteMode) {
            ...
            if (PestsDestroyer.getInstance().isRunning() || TaunahiConfig.pestsDestroyerAfkInfiniteMode) {
                if (TaunahiConfig.pestsDestroyerAfkInfiniteMode) {
                    LogUtils.sendWarning("[Pests Destroyer] Disabling AFK Mode!");
                    TaunahiConfig.pestsDestroyerAfkInfiniteMode = false;
                }
    ...
    public void enableMacro() {
        ...
        if (TaunahiConfig.sendMacroEnableDisableLogs) {
            LogUtils.webhookLog("Macro enabled!");
        }

        analyticsTimer.reset();
        Multithreading.schedule(() -> {
            if (!macroingTimer.isScheduled() || TaunahiConfig.resetStatsBetweenDisabling) {
                ...
                if (message.equals("Your spawn location has been set!")) {
                    ...
                    for (Rewarp rewarp : TaunahiConfig.rewarpList) {
                        if (mc.thePlayer.getDistance(rewarp.x, rewarp.y, rewarp.z) < 2) {
                            ...
                            TaunahiConfig.rewarpList.remove(rewarp);
                            break;
                        }
                    }
                }
    ...
    private void checkForTeleport() {
        ...
        if (mc.thePlayer.getPosition().distanceSq(beforeTeleportationPos.get()) > 2 || PlayerUtils.isStandingOnSpawnPoint()) {
            ...
            currentMacro.ifPresent(cm -> {
                ...
                cm.setRotated(false);
                cm.getRewarpDelay().schedule(TaunahiConfig.getRandomTimeBetweenChangingRows());
                if (rewarpTeleport) {
