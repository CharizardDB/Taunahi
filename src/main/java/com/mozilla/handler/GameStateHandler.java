package com.mozilla.handler;

import com.mozilla.config.TaunahiConfig;
import com.mozilla.event.*;
import com.mozilla.failsafe.FailsafeManager;
import com.mozilla.failsafe.impl.CobwebFailsafe;
import com.mozilla.failsafe.impl.DirtFailsafe;
import com.mozilla.failsafe.impl.JacobFailsafe;
import com.mozilla.feature.impl.AutoRepellent;
import com.mozilla.feature.impl.PestsDestroyer;
import com.mozilla.util.*;
import com.mozilla.util.helper.Clock;
import com.mozilla.util.helper.Timer;
...
public class GameStateHandler {
    ...
    @Getter
    private List<TaunahiConfig.CropEnum> jacobsContestNextCrop = new ArrayList<>();
    @Getter
    ...
    public void onTablistUpdate(UpdateTablistEvent event) {
        ...
        List<TaunahiConfig.CropEnum> newJacobsContestNextCrop = new ArrayList<>();
        ...
        for (String cleanedLine : tabList) {
            ...
            if (nextJacobCropFound >= 0 && nextJacobCropFound < 3) { // Make sure only 3 crops are added and no irrelevant text are being scanned
                TaunahiConfig.CropEnum crop = convertCrop(cleanedLine);
                if (crop != TaunahiConfig.CropEnum.NONE && !newJacobsContestNextCrop.contains(crop))
    ...
    private TaunahiConfig.CropEnum convertCrop(String s) {
        if (s.contains("Wheat")) {
    ...
    private void checkJacob(String cleanedLine) {
        ...
        if (isInJacobContest) {
            ...
            if (!jacobContestLeftClock.isScheduled() || !jacobsContestCrop.isPresent()) {
                ...
                if (matcher.find()) {
                    TaunahiConfig.CropEnum crop = convertCrop(cleanedLine);
                    if (crop != TaunahiConfig.CropEnum.NONE)
        ...
        for (String cleanedLine : list) {
            if (cleanedLine.contains("The Garden") && cleanedLine.contains("ൠ")) {
                try {
                    ...
                    pestsCount = temp;
                    pestsCountTemp = temp;
                    if (pestsCount > previousPestsCount && !PestsDestroyer.getInstance().isRunning() && pestsCount > TaunahiConfig.startKillingPestsAt) {
                        if (TaunahiConfig.sendWebhookLogIfPestsDetectionNumberExceeded) {
                            LogUtils.webhookLog("[Pests Destroyer]\\nThere " + (pestsCount > 1 ? "are" : "is") + " currently **" + pestsCount + "** " + (pestsCount > 1 ? "pests" : "pest") + " in the garden!", TaunahiConfig.pingEveryoneOnPestsDetectionNumberExceeded);
                        }
    ...
    public boolean canRewarp() {
        if (randomRewarpValueToWait == -1) randomRewarpValueToWait = TaunahiConfig.getRandomRewarpDelay();
    ...
    public boolean hasPassedSinceStopped() {
        if (randomValueToWait == -1) randomValueToWait = TaunahiConfig.getRandomTimeBetweenChangingRows();
    ...
    public boolean inJacobContest() {
        ...
        if (TaunahiConfig.jacobContestCurrentCropsOnly) {
