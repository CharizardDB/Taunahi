...
import com.mozilla.util.MarkdownFormatter;
...
public class AutoUpdaterGUI extends GuiScreen {
    ...
    private static boolean isOutdated() {
        ...
        Matcher currentVersionMatcher = versionPattern.matcher(BufferedCipher.VERSION);
    ...
    private static int compareSymVer(String latestVersion) {
        Matcher currentMatcher = versionPattern.matcher(BufferedCipher.VERSION);
    ...
    private static void deleteAndClose() {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            File currentModsFolder = BufferedCipher.jarFile.getParentFile();
                    ...
                    executor.submit(() -> {
                        try {
                            ...
                            downloadFileWithProgress(
                                    downloadURL,
                                    new File(mc.mcDataDir + "/mods/TaunahiV2-" + latestVersion + ".jar")
                            );
                            mc.addScheduledTask(() -> {
                                downloadBtn.enabled = false;
